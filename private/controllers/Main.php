...
// ----------------------------------pagination-----------------------
            ...
            if(isset($_POST['search'])){//=================================if search bar selected
                ...
                $q1 = "select * from donation_camp_request where status=0 AND blood_bank=:bbid AND name like '$text%'";
                $data = $bdc->query($q1,[
                    'bbid'=>$bbid,
                ]);
            ...
            if(isset($_POST['search'])){//=================================if search bar selected
                $text=$_POST['searchval'];
                $q1 = "select * from donation_camp_request where status=0 AND blood_bank=:bbid AND name like '$text%' limit $thispagefirstres,$resultsperpage";
                $data2 = $bdc->query($q1,[
                    'bbid'=>$bbid,
                ]);
